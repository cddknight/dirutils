 ...
 *  @version $Id: hexDump.c 1076 2010-10-02 20:38:45Z chris $
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
	...
	if ((readFile = fopen ((char *)inFile, "rb")) != NULL)
	{
		...
		while ((read = fread (inBuffer, 1, 1024, readFile)) != 0)
		{
			...
			while (i < read)
			{
				...
				if (j == 16)
				{
					displayInColumn (0, "%06X", filePosn);
					displayInColumn (1, "%s", &saveHex[1]);
					displayInColumn (2, "%s", saveChar);
					displayNewLine(0);
