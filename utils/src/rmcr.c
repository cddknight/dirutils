...
void version (void)
{
	printf ("TheKnight: Remove Carrage Return, Version %s\n", directoryVersion());
	displayLine ();
}
...
int main (int argc, char *argv[])
{
     ...
     *------------------------------------------------------------------------*/
	while (i < argc)
		found += directoryLoad (argv[i++], ONLYFILES, NULL, &fileList);
	...
	if (found)
	{
		...
		displayInColumn (1, "CR's removed");
